...
def refine_centroid(image: NDArray[np.float32], initial_position: Tuple[float, float], box_size: int=51, centroid_func=centroid_2dg) -> Tuple[float, float]:
    ...
    try:
        with warnings.catch_warnings():
            warnings.filterwarnings('ignore', message='.*fit may not have converged.*', category=Warning)
            x_refined, y_refined = centroid_sources(image, [x_init], [y_init], box_size=box_size, centroid_func=centroid_func)
        x_ref, y_ref = (float(x_refined[0]), float(y_refined[0]))
        if np.isnan(x_ref) or np.isnan(y_ref) or (not 0 <= x_ref < w) or (not 0 <= y_ref < h):
            x_com, y_com = centroid_sources(image, [x_init], [y_init], box_size=box_size, centroid_func=centroid_com)
...
def optimize_aperture_radius(image: NDArray[np.float32], position: Tuple[float, float], radii: NDArray[np.float64], annulus_inner: float, annulus_outer: float, ccd_gain: float=1.0, return_snr_curve: bool=False) -> float:
...
def measure_flux(image: NDArray[np.float32], position: Tuple[float, float], aperture_radius: float, annulus_inner: float, annulus_outer: float, ccd_gain: float=1.0, error_map: Optional[NDArray[np.float64]]=None) -> dict:
    ...
    bkg_mean = float(np.median(annulus_data_1d))
    bkg_std = float(np.std(annulus_data_1d))
    ...
    return {'flux': float(flux), 'flux_err': float(flux_err), 'background_mean': bkg_mean, 'background_std': bkg_std, 'snr': snr, 'aperture_sum': aperture_sum, 'centroid': refined_position}

class ApertureConfig:

    def __init__(self, aperture_radius: float, annulus_inner: float, annulus_outer: float, ccd_gain: float=1.0, centroid_box_size: int=51) -> None:
    ...
    def measure_flux(self, image: NDArray[np.float32], position: Tuple[float, float]) -> dict:
        return measure_flux(image, position, self.aperture_radius, self.annulus_inner, self.annulus_outer, self.ccd_gain)

    def __repr__(self) -> str:
        return f'ApertureConfig(aperture_r={self.aperture_radius:.1f}, annulus={self.annulus_inner:.1f}–{self.annulus_outer:.1f} px, gain={self.ccd_gain:.2f})'
