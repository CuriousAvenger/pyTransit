...
def detect_sources(image: NDArray[np.float32], fwhm: float=5.0, threshold: float=10.0, threshold_type: str='absolute', background_std: Optional[float]=None, exclude_border: bool=True, sort_by: str='flux') -> Table:
    if image.ndim != 2:
    ...
    if threshold_type == 'sigma':
        ...
        raise ValueError(f"Unknown threshold_type '{threshold_type}'. Choose 'absolute' or 'sigma'.")
    daofind = DAOStarFinder(fwhm=fwhm, threshold=actual_threshold, exclude_border=exclude_border)
    sources = daofind(image)
    if sources is None or len(sources) == 0:
        raise RuntimeError(f'No sources detected (threshold={actual_threshold:.1f}, fwhm={fwhm}). Try lowering the threshold or adjusting FWHM.')
    ...
    print(f"  Brightest: flux={sources['flux'][0]:.0f}, position=({sources['x_centroid'][0]:.1f}, {sources['y_centroid'][0]:.1f})")
    return sources

def filter_sources(sources: Table, min_sharpness: float=0.3, max_sharpness: float=1.0, max_roundness: float=0.5, min_flux: Optional[float]=None) -> Table:
...
def select_reference_stars(sources: Table, target_index: int, n_references: int=3, max_separation: Optional[float]=None) -> Tuple[Tuple[float, float], List[Tuple[float, float]], List[int]]:
    if target_index >= len(sources):
        raise ValueError(f'target_index {target_index} out of range (table has {len(sources)} rows)')
    ...
    if max_separation is not None:
        candidate_indices = [idx for idx in candidate_indices if np.hypot(float(sources['x_centroid'][idx]) - target_x, float(sources['y_centroid'][idx]) - target_y) <= max_separation]
    if len(candidate_indices) < n_references:
        warnings.warn(f'Only {len(candidate_indices)} reference stars available (requested {n_references})')
        n_references = len(candidate_indices)
    reference_indices = candidate_indices[:n_references]
    reference_positions = [(float(sources['x_centroid'][idx]), float(sources['y_centroid'][idx])) for idx in reference_indices]
    ...
    for i, (idx, pos) in enumerate(zip(reference_indices, reference_positions)):
        print(f"    Ref {i + 1} (star #{idx}): ({pos[0]:.1f}, {pos[1]:.1f}), flux={float(sources['flux'][idx]):.0f}")
