...
def detect_sources(
    ...
    print(
        f"  Brightest source: flux={sources['flux'][0]:.0f}, "
...
def select_reference_stars(
    sources: Table, target_index: int, n_references: int = 3, max_separation: Optional[float] = None
) -> tuple:
    """
    ...
    reference_positions = [
        (sources["xcentroid"][idx], sources["ycentroid"][idx]) for idx in reference_indices
    ]
    ...
    for i, (idx, pos) in enumerate(zip(reference_indices, reference_positions)):
        print(
            f"    Ref {i+1} (star #{idx}): position=({pos[0]:.1f}, {pos[1]:.1f}), "
...
def estimate_background(
    image: np.ndarray, sample_size: int = 100, method: str = "corners"
) -> tuple:
    """
