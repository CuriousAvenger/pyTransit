...
def plot_lightcurve(
    ...
    ax.axhline(
        y=baseline, color="gray", linestyle="--", lw=2, alpha=0.5, label=f"Median = {baseline:.4f}"
    )
...
def plot_sigma_clipping(
    ...
    ax.set_ylabel("Flux Ratio", fontsize=11, weight="bold")
    ax.set_title(
        f"After {sigma_threshold}σ-Clipping ({len(times_after)} points)", fontsize=12, weight="bold"
    )
    ax.grid(True, alpha=0.3)
