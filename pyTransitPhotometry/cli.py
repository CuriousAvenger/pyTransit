...
def main() -> None:
    parser = argparse.ArgumentParser(description='Transit Photometry Pipeline - Exoplanet transit analysis', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n  # Run full pipeline\n  transit-pipeline config.yaml\n\n  # Create example configuration\n  transit-pipeline --create-config my_config.yaml\n\n  # Run with custom output directory\n  transit-pipeline config.yaml --output ./my_results\n\n  # Run individual stages\n  transit-pipeline config.yaml --stages calibration detection photometry\n        ')
    parser.add_argument('config', nargs='?', help='Path to YAML configuration file')
    parser.add_argument('--create-config', metavar='PATH', help='Create an example configuration file at PATH')
    parser.add_argument('--output', '-o', metavar='DIR', help='Override output directory from config')
    parser.add_argument('--stages', '-s', nargs='+', choices=['calibration', 'detection', 'photometry', 'detrending', 'fit', 'export'], help='Run only specified stages (default: all)')
