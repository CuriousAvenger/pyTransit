...
def differential_photometry(
    ...
    ratio_err = ratio * np.sqrt(
        (target_err / target_flux)**2 +
        (weighted_ref_err / weighted_ref_flux)**2
    )
...
class LightCurveBuilder:
    ...
    def __init__(
        ...
        print(f"✓ Light curve builder initialized")
        ...
        for i in range(n_frames):
            try:
                ...
                if max(required_indices) >= n_detected:
                    if verbose:
                        print(f"⚠  Frame {i+1}/{n_frames}: Missing stars (only {n_detected} detected), skipping")
                ...
                for ref_idx in self.reference_indices:
                    try:
                        ...
                        if verbose:
                            print(f"⚠  Frame {i+1}/{n_frames}: Reference star {ref_idx} failed: {e}")
                        continue
                ...
                ratio, ratio_err = differential_photometry(
                    target_flux, target_err,
                    np.array(ref_fluxes), np.array(ref_errs),
