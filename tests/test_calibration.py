"""Tests for CCD calibration routines (calibration.py)."""
...
class TestScaleDarkFrame:
    def test_linear_scaling(self):
        ...
        dark = flat_image(600.0)  # 100 counts at 10 s → 10 counts/s
        scaled = scale_dark_frame(dark, bias, dark_exptime=10.0, target_exptime=85.0)
        assert np.allclose(scaled, 850.0, atol=1e-3), f"Expected 850.0, got {scaled.mean():.2f}"
    ...
    def test_invalid_exptime_raises(self):
        ...
        with pytest.raises(ValueError):
            scale_dark_frame(
                np.ones((10, 10)), np.zeros((10, 10)), dark_exptime=-1.0, target_exptime=85.0
            )


# ===========================================================================
# TestCreateNormalizedFlat
# ===========================================================================
...
class TestCreateNormalizedFlat:
    def test_unit_mean(self):
        ...
        assert np.allclose(normalized, 1.0, atol=1e-5), f"Flat mean {normalized.mean():.6f} ≠ 1.0"
