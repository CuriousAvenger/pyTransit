"""Tests for transit model fitting: batman injection/recovery (models.py)."""

import numpy as np
import pytest


# Shared fit parameters and bounds for WASP-75b-like injection
_INIT_PARAMS = {"rp": 0.10, "a": 7.0, "inc": 83.0, "baseline": 1.0, "slope": 0.0}
_BOUNDS = {
    "rp": (0.07, 0.14),
    "a": (5.0, 10.0),
    "inc": (75.0, 90.0),
...
class TestTransitInjectionRecovery:
    ...
    def test_fit_converges(self, synthetic_transit):
        ...
        result = self._make_fitter().fit(
            times, fluxes, errors, initial_params=_INIT_PARAMS, bounds=_BOUNDS, fix_t0=True
        )
    ...
    def test_rp_recovery(self, synthetic_transit):
        ...
        assert abs(rp_fit - self.TRUTH["rp"]) < tol, (
            f"Rp/Rs recovery failed: fit={rp_fit:.5f}, "
            f"truth={self.TRUTH['rp']:.5f}, tol={tol:.5f}"
        )
    ...
    def test_inclination_recovery(self, synthetic_transit):
        ...
        assert abs(inc_fit - self.TRUTH["inc"]) < tol, (
            f"Inclination recovery failed: fit={inc_fit:.3f}°, "
            f"truth={self.TRUTH['inc']:.3f}°, tol={tol:.3f}°"
        )
    ...
    def test_semi_major_axis_recovery(self, synthetic_transit):
        ...
        tol = max(3 * a_err, 0.5)
        assert abs(a_fit - self.TRUTH["a"]) < tol, (
            f"a/Rs recovery failed: fit={a_fit:.4f}, truth={self.TRUTH['a']:.4f}, tol={tol:.4f}"
        )

    def test_fit_from_off_initial_guess(self, synthetic_transit):
        """Fitter must converge even with initial guess 10% off truth."""
        times, fluxes, errors = synthetic_transit
        truth = self.TRUTH
        off_params = {
            "rp": truth["rp"] * 1.10,
        ...
        result = self._make_fitter().fit(
            times, fluxes, errors, initial_params=off_params, bounds=off_bounds, fix_t0=True
        )
        assert "fitted_params" in result, "Fit should return fitted_params"
        rp_fit = result["fitted_params"]["rp"][0]
        assert (
            abs(rp_fit - truth["rp"]) < 0.015
