...
class TestSigmaClip:
    ...
    def clean_lc(self):
        ...
        fluxes = 1.0 + RNG.normal(0, 0.001, 150)
        errors = np.full(150, 0.001)
...
class TestRollingMADFilter:
    ...
    def clean_lc(self):
        ...
        fluxes = 1.0 + RNG.normal(0, 0.0005, 200)
        errors = np.full(200, 0.0005)
        return (times, fluxes, errors)
    ...
    def test_rejects_tracking_spikes(self, clean_lc):
        ...
        fluxes_spiked[spike_idx] = 2.0
        _, _, _, mask = rolling_mad_filter(times, fluxes_spiked, errors, window_size=15, sigma_mad=3.0)
        for idx in spike_idx:
    ...
    def test_protects_transit_ingress(self, clean_lc):
        ...
        _, _, _, mask = rolling_mad_filter(times, fluxes_transit, errors, window_size=20, sigma_mad=3.5)
...
class TestIsolationForestFilter:
    ...
    def clean_lc(self):
        ...
        fluxes = 1.0 + RNG.normal(0, 0.0005, 300)
        errors = np.full(300, 0.0005)
        return (times, fluxes, errors)
...
class TestHuberAirmassDetrend:
    ...
    def airmass_lc(self):
        times = np.linspace(0, 2, 200)
        airmass = 1.0 + 0.5 * times
        fluxes = 1.0 - 0.03 * (airmass - 1.0) + RNG.normal(0, 0.0005, 200)
        errors = np.full(200, 0.0005)
