"""Tests for photometry routines: 2D background estimation and flux measurement."""
...
class TestMeasureFlux:
    ...
    def test_flux_positive(self, star_image):
        ...
        result = measure_flux(
            image, position, aperture_radius=8.0, annulus_inner=15.0, annulus_outer=25.0
        )
    ...
    def test_off_target_lower_flux(self, star_image):
        ...
        image, position = star_image
        on_target = measure_flux(
            image, position, aperture_radius=8.0, annulus_inner=15.0, annulus_outer=25.0
        )
        off_target = measure_flux(
            image,
            (35.0, 35.0),  # no star here
            aperture_radius=8.0,
            annulus_inner=15.0,
            annulus_outer=25.0,
        )
        assert (
            on_target["flux"] > off_target["flux"]
